...
class Error:
    ...
    def __repr__(self):
        result = f"{self.error_name}: {self.description}\n"
        result += f"File {self.pos_start.fn}, line {self.pos_end.ln + 1}"
        result += "\n\n" + strings.arrows(self.pos_start.ftxt, self.pos_start, self.pos_end)
        return result
...
class ExpectedCharError(Error):
    def __init__(self, pos_start, pos_end, description):
        super().__init__(pos_start, pos_end, "Expected Character", description)


class InvalidSyntaxError(Error):
    def __init__(self, pos_start, pos_end, description=""):
        super().__init__(pos_start, pos_end, "Invalid Syntax", description)
    ...
    def expr(self):
        ...
        if self.current_tok.matches(TT_KEYWORD, "VAR"):
            ...
            if self.current_tok.type != TT_IDENTIFIER:
                return res.failure(InvalidSyntaxError(self.current_tok.pos_start, self.current_tok.pos_end, "Expected identifer"))
...
class Interpreter:
    ...
    def visit_VarAccessNode(self, node, context):
        ...
        if not value:
            return res.failure(RTError(node.pos_start, node.pos_end, f"Name {var_name} is not defined", context))
